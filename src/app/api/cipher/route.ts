...
- Use only existing citations [${availableIds}]`
    ...
    try {
      ...
      if (!response) {
        return NextResponse.json(
          { error: 'Failed to generate edit response' },
          { status: 500 }
        )
      }

      // Clean up the response using cleanJsonResponse and remove control characters
      let cleanedResponse = cleanJsonResponse(response).replace(/[\x00-\x1F]+/g, ' ');

      try {
        // First try direct parse on the cleaned response
        const edit = JSON.parse(cleanedResponse);
        if (edit?.editedContent) {
          return processEdit(edit.editedContent, existingSources);
        }
      } catch (e) {
        // If direct parse fails, try to extract just the JSON object
